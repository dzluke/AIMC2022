...
def frame_distance(custom_distance_metric):
    ...
    def custom_distance(target, solution):
        ...
        distances = []
        target_energy = []
        num_frames = min(target_frames.shape[0], solution_frames.shape[0])  # only go as far as the shorter audio file
        ...
        avg_energy = energy_sum / energy.shape[0]
        weighted_sum = np.sum(distances * energy) / energy_sum
...
def cosine_similarity(target, solution):
    """
    Calculates the cosine similarity between target and solution. Higher values are better
...
def cosine_distance(target, solution):
    """
    Calculates the cosine distance between target and solution
    cosine_distance(X, Y) = 1 - [ <X, Y> / (||X||*||Y||) ]
    :param target: audio, shape: (len,)
    :param solution: audio, shape: (len,)
    :return: scalar representing the distance
    """
    return 1 - cosine_similarity(target, solution)
    ...
    while num_completed < len(targets):
        ...
        for model, subtargets in all_subtargets.items():  # for each separation algorithm
            ...
            for j in range(len(subtargets)):  # for each subtarget
                ...
                # new orchestration evaluation
                assert len(solutions) == 1, "New orch eval does not support more than 1 threshold"
                solution = solutions[0]

                # apply thresholding based on RMS of subtarget
                # if RMS(subtarget) is too low, don't calculate the distance
                RMS_THRESHOLD = 0.05
                RMS = np.sqrt(np.mean(solution**2))  # root mean square
                if RMS >= RMS_THRESHOLD:
                    orchestrated_subtargets[model].append(solutions)
